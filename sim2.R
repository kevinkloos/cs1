######################### FUNCTIONS #########################
simulate_test <- function(n_obs, sdev_pos, sdev_neg, prev, prev_train = NULL) {
  ...
  .raw_to_prob <- function(dat, prev, sdev_pos, sdev_neg) {
    p1 <- prev * dnorm(dat, mean = 1, sd = sdev_pos)
    p2 <- (1 - prev) * dnorm(dat, mean = 0, sd = sdev_neg)
    return(p1 / (p1 + p2))
  }
  
  prob_pos <- .raw_to_prob(pos, prev_train, sdev_pos, sdev_neg)
  prob_neg <- .raw_to_prob(neg, prev_train, sdev_pos, sdev_neg)
...
simulate_train <- function(n_obs, sdev_pos, sdev_neg, prev) {
  stopifnot("n_obs * prev should be an integer" = ((n_obs * prev) %% 1) == 0)
  n_pos <- n_obs * prev
  n_neg <- n_obs - n_pos
  
  data_pos <- rnorm(n_pos, mean = 1, sd = sdev_pos)
  data_neg <- rnorm(n_neg, mean = 0, sd = sdev_neg)
  ...
  cs_pos <- c("mean" = mean_pos, "sd" = sd_pos)
  cs_neg <- c("mean" = mean_neg, "sd" = sd_neg)
  ...
  .raw_to_prob <- function(dat, prev = prev, sdev_pos = sdev_pos, sdev_neg = sdev_neg) {
    p1 <- prev * dnorm(dat, mean = 1, sd = sdev_pos)
    p2 <- (1 - prev) * dnorm(dat, mean = 0, sd = sdev_neg)
    return(p1 / (p1 + p2))
  }
  
  prob_pos <- .raw_to_prob(data_pos, prev, sdev_pos, sdev_neg) |> sort()
  prob_neg <- .raw_to_prob(data_neg, prev, sdev_pos, sdev_neg) |> sort()
...
one_sim_function <- function(n_train, n_test, sdev_pos, sdev_neg,
                             prev_train, prev_test, reps) {
  
  out <- map(1:reps, function(x) {
    train <- simulate_train(n_train, sdev_pos, sdev_neg, prev_train)
    test <- simulate_test(n_test, sdev_pos, sdev_neg, prev_test, prev_train)
    prevs <- get_prevalence_estimates(train, test) 
    return(prevs)
  })
